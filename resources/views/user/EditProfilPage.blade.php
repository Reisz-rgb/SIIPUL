            ...
            <div class="p-6 md:p-8 border-b border-slate-50">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center">
                        <span class="text-[var(--maroon)] text-lg font-extrabold">{{ strtoupper(substr(($authUser->name ?? 'U'), 0, 2)) }}</span>
                    </div>
                    <div>
                        <h3 class="text-lg md:text-xl font-extrabold text-slate-800">{{ $authUser->name ?? '-' }}</h3>
                        <p class="text-sm text-slate-500 font-semibold">{{ $authUser->jabatan ?? 'Pegawai' }} • {{ $authUser->bidang_unit ?? '-' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 md:p-8">
                ...
                <form action="{{ route('user.profil.update') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" value="{{ old('phone', $authUser->phone) }}" required
                               class="w-full px-4 py-3 rounded-2xl border border-slate-200 bg-white text-slate-800 font-semibold focus:outline-none focus:ring-4 focus:ring-red-100 focus:border-[var(--maroon)]"
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $authUser->nip }}" readonly
                    ...
                    <div>
                        ...
                        <input type="text" name="jabatan" value="{{ old('jabatan', $authUser->jabatan) }}"
                               class="w-full px-4 py-3 rounded-2xl border border-slate-200 bg-white text-slate-800 font-semibold focus:outline-none focus:ring-4 focus:ring-red-100 focus:border-[var(--maroon)]"
                    ...
                    <div>
                        ...
                        <input type="text" name="bidang_unit" value="{{ old('bidang_unit', $authUser->bidang_unit) }}"
                               class="w-full px-4 py-3 rounded-2xl border border-slate-200 bg-white text-slate-800 font-semibold focus:outline-none focus:ring-4 focus:ring-red-100 focus:border-[var(--maroon)]"
