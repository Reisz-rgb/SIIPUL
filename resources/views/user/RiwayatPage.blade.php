        ...
        <div class="p-6 md:p-7 border-b border-slate-50 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('user.riwayat') }}"
                   class="px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all {{ $status == 'all' ? 'active-menu' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' }}">
                    Semua
                </a>
                <a href="{{ route('user.riwayat', ['status' => 'approved']) }}"
                   class="px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all {{ $status == 'approved' ? 'active-menu' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' }}">
                    Disetujui
                </a>
                <a href="{{ route('user.riwayat', ['status' => 'pending']) }}"
                   class="px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all {{ $status == 'pending' ? 'active-menu' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' }}">
                    Tertunda
                </a>
                <a href="{{ route('user.riwayat', ['status' => 'rejected']) }}"
                   class="px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider border transition-all {{ $status == 'rejected' ? 'active-menu' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50' }}">
                    Ditolak
                </a>
            </div>
        </div>
        ...
        <div class="w-full max-w-3xl bg-white rounded-2xl overflow-hidden shadow-2xl border border-black/10 flex flex-col max-h-[90vh]">
            ...
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                ...
                <button @click="closeModal()" type="button"
                        class="w-10 h-10 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 text-xl flex items-center justify-center">
                    ×
                </button>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <input :value="data.id"
                               type="text"
                               readonly
                               class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-600" />
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input :value="data.status"
                               ...
                               type="text"
                               readonly
                               class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-extrabold" />
                    </div>

                    {{-- Jenis Cuti --}}
                    <div class="space-y-2">
                        <label class="text-xs font-extrabold text-slate-600">Jenis Cuti</label>
                        <select x-model="data.jenis"
                                disabled
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm shadow-sm">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col min-w-0">
                                    ...
                                    <div class="mt-0.5 truncate font-extrabold text-[var(--maroon)]"
                    ...
                    <div class="space-y-2">
                        ...
                        <input :value="data.kontakDarurat"
                               type="text"
                               readonly
                               class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" />
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input :value="data.hubunganDarurat"
                               type="text"
                               readonly
                               class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm" />
                    </div>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <textarea x-bind:value="data.alasan"
                                  readonly
                                  class="w-full min-h-[110px] rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm"></textarea>
                    </div>

                    {{-- Catatan Admin --}}
                    <div class="space-y-2 md:col-span-2">
                        <label class="text-xs font-extrabold text-slate-600">Catatan Admin</label>
                        <textarea x-bind:value="data.catatan"
                                  readonly
                                  class="w-full min-h-[110px] rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3 text-sm"></textarea>
                    </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <button @click="closeModal()"
                        type="button"
                        class="px-5 py-3 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-sm font-extrabold text-slate-700">
                    Tutup
                </button>
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('user.cuti.create') }}"
                       x-show="status === 'Ditolak'"
                       class="sm:inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-sm font-extrabold text-white btn-primary">
